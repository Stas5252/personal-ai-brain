...
ACTIONS = {
    ...
    '✨ Разбор аккаунта': 'Сделай профессиональный аудит моего профиля или пришли скриншот: шапка, УТП, навигация актуального и шахматный ритм ленты по крупности планов.',
    '🔍 Разбор фото': 'Разбери мою фотографию: свет, композиция, эмоция, цвет и 3 конкретных шага, как сделать кадр в 2 раза сильнее.',
    '🎵 Подбор треков': 'Подбери 4-5 атмосферных треков для фотосессии, Reels или Stories с художественным объяснением, почему они подходят под визуал.',
    '⚖️ Спорная ситуация': 'Помоги разобрать спорную или конфликтную ситуацию с клиентом: где моя просадка, в чем права клиентка, где она перегибает и готовый сдержанный скрипт ответа.',
    ...
    def reply(self, message):
        ...
        try:
            ...
            if message.get('photo'):
                ...
                if any(w in c_low for w in ['профиль', 'аккаунт', 'шапк', 'лент', 'сетк', 'инста', 'хайлайтс', 'аудит']):
                    text = caption or 'Сделай детальный аудит профиля и ленты по скриншоту: шапка, УТП, актуальное и шахматный ритм планов.'
                elif any(w in c_low for w in ['спор', 'конфликт', 'клиент', 'переписк', 'претензи', 'диалог']):
                    text = caption or 'Помоги разобрать диалог с клиентом на скриншоте: стадия, скрытые сомнения и стратегия ответа без скидок.'
                else:
