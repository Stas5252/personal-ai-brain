"""Processing state machine for knowledge ingestion and resumable checkpoints."""
...
VALID_TRANSITIONS = {
    ...
    IngestionStatus.COMPLETED: {IngestionStatus.DISCOVERED},
...
class IngestionStateMachine:
    ...
    def can_resume(self) -> Tuple[bool, Optional[IngestionStatus]]:
        ...
        if status in [IngestionStatus.FAILED, IngestionStatus.RETRY_PENDING] and checkpoint:
            try:
                return True, IngestionStatus(checkpoint)
            except ValueError:
                return False, None
        return False, None
