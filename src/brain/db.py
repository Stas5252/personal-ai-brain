...
def init_db():
    ...
    cursor.execute("PRAGMA table_info(knowledge_sources)")
    existing = {row["name"] for row in cursor.fetchall()}
    for name, column_type in [
        ("original_filename", "TEXT"), ("mime_type", "TEXT"),
        ("file_size", "INTEGER DEFAULT 0"), ("sha256", "TEXT"), ("p_hash", "TEXT"),
        ("storage_path", "TEXT"), ("derived_dir", "TEXT"),
        ("ingestion_status", "TEXT DEFAULT 'COMPLETED'"),
        ("processing_version", "TEXT DEFAULT 'v1.0'"),
        ("classification_method", "TEXT DEFAULT 'rules'"),
        ("seen_count", "INTEGER DEFAULT 1"), ("checkpoint_stage", "TEXT"),
        ("error_message", "TEXT"), ("metadata_json", "TEXT DEFAULT '{}'")
    ]:
        if name not in existing:
            cursor.execute(f"ALTER TABLE knowledge_sources ADD COLUMN {name} {column_type}")
    ...
    for name, column_type in [
        ...
        ("end_time", "REAL"), ("heading_path", "TEXT"),
        ("language", "TEXT DEFAULT 'ru'"), ("token_count", "INTEGER DEFAULT 0")
    ]:
        if name not in existing:
            cursor.execute(f"ALTER TABLE knowledge_chunks ADD COLUMN {name} {column_type}")
    ...
    for name, column_type in [
        ...
        if name not in existing:
            cursor.execute(f"ALTER TABLE ingestion_jobs ADD COLUMN {name} {column_type}")
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_ij_lease ON ingestion_jobs(status, lease_until, priority DESC)")
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_ij_worker ON ingestion_jobs(worker_id)")
    ...
    for name, column_type in [
        ...
        if name not in existing:
            cursor.execute(f"ALTER TABLE clients ADD COLUMN {name} {column_type}")
    ...
    for name, column_type in [
        ...
        if name not in existing:
            cursor.execute(f"ALTER TABLE projects ADD COLUMN {name} {column_type}")
    ...
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_tasks_status ON tasks(status, priority, due_at)")
    ...
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_wf_status ON workflows(status, updated_at)")
    cursor.execute(
