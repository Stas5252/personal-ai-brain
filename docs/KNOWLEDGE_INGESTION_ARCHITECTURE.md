...
| :--- | :--- | :--- |
| **DocumentExtractor** | `.pdf`, `.docx`, `.pptx`, `.xlsx`, `.txt`, `.md`, `.html` | **PDF**: Native text extraction with automatic fallback for scanned pages (extracting page image to derived artifacts + OCR sidecar loading).<br>**DOCX**: Paragraph heading hierarchy (`heading_path`), atomic table structure preservation, list parsing.<br>**PPTX**: Slide-by-slide boundary preservation with titles, bullets, and speaker notes.<br>**XLSX**: Multi-sheet tabular parsing preserving sheet names, columns, and rows.<br>**HTML/MD**: Navigation/footer boilerplate removal, header tree retention. |
| **ImageExtractor** | `.jpg`, `.jpeg`, `.png`, `.webp`, `.tiff` | Strict separation of **factual OCR** from **AI vision descriptions** (to prevent hallucinations). Technical EXIF extraction (camera make, model, focal length, ISO, exposure time). Structured visual metadata (`objects`, `tags`). |
...
- **Hallucination Refusal Gate**: If keyword overlap is below 25% and vector similarity is below 0.60, the chunk is rejected as a candidate, guaranteeing reliable refusal on unknown facts.
